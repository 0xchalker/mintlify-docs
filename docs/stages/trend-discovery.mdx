...
| Output khóa | `review-scope.md`, `batch-audit.md`, `trend-final.md`, `trend-package/`, `x-public-log.*` (optional) |
...
Mục tiêu của đoạn này không phải là kết luận winner, mà là tạo đủ vật liệu ban
đầu.

## 2. Parse ra từng narrative riêng

AI tách raw report thành các narrative files độc lập để có thể review từng
story riêng.
...
## 7. Viết Trend Final cho Step 2

Mục tiêu là để Step 2 không phải đọc lại toàn bộ batch từ đầu.

`trend-final.md` phải đủ rõ để cho biết:

- winner candidates là ai
- tại sao candidate này đáng đi tiếp
- đâu là điểm cần cẩn trọng
- nếu shortlist đang đồng điểm thì winner được freeze theo rule nào

Sau khi freeze winner, runtime còn phải đóng gói `trend-package/` để Step 2
đọc ngay bộ tài liệu liên quan đến narrative top và các lane dự phòng.
...
- `trend-final.md`
- `trend-package/`
- `x-public-log.json` khi public trust layer trên X được bật
- `x-public-log.md` khi public trust layer trên X được bật

## Internal flow và public flow

Step 1 giờ có hai lớp output:

- lớp nội bộ để Step 2 làm việc:
  - `review-scope.md`
  - `batch-audit.md`
  - `trend-final.md`
  - `trend-package/`
- lớp công khai để cộng đồng thấy AI đang làm gì:
  - root run post
  - raw narrative reply
...
`trend-package/` là handoff package thật.
`x-public-log.*` chỉ là trust surface và replay surface.
...
Có những lần Step 1 chạy ra batch rỗng.

Điều đó vẫn là kết quả hợp lệ nếu Grok kết luận:

- thị trường đang yên
- chưa có trigger đủ mới
- không có narrative nào đạt ngưỡng để đi tiếp

Khi đó runtime vẫn phải viết:

- `review-scope.md`
- `batch-audit.md`
- `trend-final.md`

và nói rõ đây là `empty-state` hoặc `rerun-state`, không phải lỗi im lặng.
...
- batch đã có audit
- batch đã có Trend Final
- batch đã có Trend Package
